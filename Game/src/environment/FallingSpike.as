	...
	public class FallingSpike extends Bitmap {
		...
		public function update():void {
			if (x >= Main.player.x - 800 && x <= Main.player.x + 800 && y >= Main.player.y - 600 && y <= Main.player.y + 600) {
				...
				if (coordx == Main.player.coordx || int(x / 20) == Main.player.coordx) {
					if (coordy == Main.player.coordy || int(y / 20) == Main.player.coordy) {
						remove();
						Main.player.die();
					}
				}
				if (fall) {
					...
					while (true) {
						if (coordx >= 0 && coordx < Main.map.gridwidth && coordy >= 0 && coordy < Main.map.gridheight) {
