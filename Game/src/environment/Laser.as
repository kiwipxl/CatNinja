		...
		public function switchLaser():void {
			...
			if (on) {
				...
				for (n = 0; n < (to.x - from.x) + (to.y - from.y) + 1; ++n) {
					Main.map.changeTile(x, y, 0, null, null, true);
					if (x != to.x) { ++x; } else if (y != to.y) { ++y; }
				}
				Main.map.changeTile(from.x, from.y, machinegraphicoff1, null, null, true);
				Main.map.changeTile(to.x, to.y, machinegraphicoff2, null, null, true);
			}else {
				on = true;
				for (n = 0; n < (to.x - from.x) + (to.y - from.y) + 1; ++n) {
					Main.map.changeTile(x, y, lasergraphic, null, null, true);
					if (x != to.x) { ++x; } else if (y != to.y) { ++y; }
				}
				Main.map.changeTile(from.x, from.y, machinegraphic1, null, null, true);
				Main.map.changeTile(to.x, to.y, machinegraphic2, null, null, true);
			}
		}
	}
}